...
In a radial gradient, the refractive index increases or decreases with distance perpendicular to the optical axis of the lens. Refractive power can be altered by varying lens thickness and aberrations can be controlled by altering the function of the index distribution. An example of a radial gradient is the ``Wood Lens'' where the refractive index is higher at the centre of the lens and lower towards the edges.
\begin{figure}[H]
\centering
\includegraphics[width=0.2\linewidth]{"../../FYP Vault/Media/Radial"}
...
In an axial gradient, the refractive index changes with respect to distance from a given plan. Usually, it is a plan that is orthogonal to the optical axis. Thin-layered axial GRIN structures are designed to minimise reflections and can reduce spherical aberration. An axial GRIN medium does not have focusing properties, instead it is determined by the lens surface.

\begin{figure}[H]
\centering
\includegraphics[width=0.2\linewidth]{"../../FYP Vault/Media/Axial"}
\caption[Axial Gradient]{}
\label{fig:axial}
\end{figure}


\subsubsection*{Spherical Gradients:}
Spherical gradients are ones where the gradient of refactive index has a perfectly spherical distribution. The value for refractive index decreases from a maximum at the centre of the spherical lens to a minimum at the edge. Notable examples of spherical gradients are the "Fisheye lens" model proposed by Maxwell and the "Luneberg Lens" model \cite{Luneburg2023}. Maxwell's model is limited only to rays emanating from points on the lens surface, while the Luneberg model is capable of imaging objects beyond the lens surface and has the ability to focus parallel rays to a single point \cite{Pierscionek2012}.

\begin{figure}[H]
\centering
\includegraphics[width=0.2\linewidth]{"../../FYP Vault/Media/Spherical"}
\caption[Spherical Gradient]{}
\label{fig:spherical}
\end{figure}


Despite their wide use in other areas of optics, spherical gradients only occur in rare cases, such as in the eye of an octopus \cite{Goncharov2020a}. For this reason the will not be considered for the rest of the project.
...
Various mathematical methods exist for modelling the shape of the lens, it's gradient index profile and tracing how light travels through it. The most important methods in relation to this project are outlined below.

\subsection{GRIN Lenses}

For this project the crystalline lens is constructed from a radial GRIN profile in the equatorial plane and axial profile in the saggital plane.\\
...
\begin{equation}\label{eq:Sag}
	h^2=2R(z)z-(1-k)z^2,
\end{equation}	
where $R$ is the radius of curvature of the lens and $k$ is the conic constant \cite{Flynn2024}. By rearranging Equation \eqref{eqn:GRIN} and setting $n(z,h)=n_0$, the following formula can be found:
...
\end{equation}
 ...
 Furthermore by taking the derivative of Equation \eqref{eqn:GRIN} with respect to $z$ and looking at the lens surface where $n(z,h)=n_0$, it can be found that the maximum refractive index occurs at the point $z=\frac{n_3}{2n_4}$ with value:

\begin{equation*}
	n_{max}=n_0+\frac{n_3^2}{4n_4}.
\end{equation*}

\subsubsection*{Lens Shape}

The following formulae can be used to determine the thickness, $d$, and the aperture, $D$ of the crystalline lens:

\begin{equation}\label{eq:thick}
	d=\frac{n_3}{n_4},
\end{equation}

\begin{equation}\label{eq:aper}
	D=\sqrt{\frac{n_3^2}{n_1n_4}}.
\end{equation}

When $d=D$, then $k=0$ and the lens is spherical. For $d\neq D$, then $k\neq 0$ and the lens will be elliptic or parabolic. This can then be solved with the other equations to get values for the GRIN coefficients that are needed for the elliptic lens. This will be the case when $0 > k > -1$.

\subsubsection*{Hyperbolic Lenses}

As previously mentioned, the crystalline lens of the human eye can be modelled by a hyperbolic shape. In order to achieve this, Equations \eqref{eq:thick} and \eqref{eq:aper} can be used to deduce it's height and thickness, but this time with a conic constant of $k<-1$. The anterior and posterior regions will have different values for $d,\ D\ and\ k$.\\

In order to maintain continuity across the anterior and posterior region of the lens, the central interface needs also to be curved rather than a straight line \cite{Navarro2009}. 

\begin{figure}[H]
\centering
\includegraphics[width=0.2\linewidth]{Images/sag-line}
\caption[Curved Central Interface \cite{Navarro2009}]{}
\label{fig:sag-line}
\end{figure}
...
This is the equation that is solved numerically in ray-tracing software such as Zemax. Starting with the Equations \eqref{eqn:para1}, \eqref{eqn:para2}, \eqref{eqn:para3} a ``Paraxial Ray-Tracing Equation'' can be derived \cite{Flynn2024}. This  can be reduced to take the form of the ray equation, Equation \eqref{eqn:ray}:

\begin{equation}\label{eqn:paraxconor}
	n\frac{\text{d}^2h}{\text{d}z^2}+\frac{\text{d}n}{\text{d}z}\frac{\text{d}h}{\text{d}z}+2n_1h=0.
\end{equation}
...
\subsubsection*{Finite Ray-Tracing}

It was further found that a differential ray equation can be found for use in the non paraxial region using methods of Lagrangian optics \cite{Flynn2024}.\\
...
This is the final form of the ``Non-Paraxial Ray-Tracing Equation''. We can solve this equation using the Frobenius method to approximate a solution using and infinite series:\\

\textbf{[ADD FULL FROBENIUS DERIVATION HERE]}

\begin{equation}
\therefore h(z)=\sum^\infty_{m=0}a_mz^m+\sum^\infty_{m=0}b_mz^m.
\end{equation}

Here $a_m,b_m$ are Frobenius coefficients.
\newpage
\printbibliography
\end{document}